...
load('../../data/processed/log_reg_transition_probability.mat','LOGTranProb')
...
for ix = 1:n_x
  for iy = 1:n_y
    LOGTranProb(:,ix,iy) = ...
      LOGTranProb(:,ix,iy)./sum(LOGTranProb(:,ix,iy));
  end
end
    ...
    for iy = 1:n_y
      for direction=1:4 %[E N W S] = [1 2 3 4]
        ...
        new_prob_dens(next_x,next_y) = new_prob_dens(next_x,next_y) ...
          + LOGTranProb(direction,ix,iy)*prob_dens(ix,iy);
      end
    end
  end
...
colorbar('Ticks',cc,'TickLabels',...
  {'10^{-8}','10^{-7}','10^{-6}','10^{-5}','10^{-4}'},'FontSize',14);
hold on
plot(forest(:,1), forest(:,2), 'ok','markersize',4)
set(gca,'position',[0.05 0.05 .8 .9],'units','normalized')

print('-depsc2','-r200','../../figures/log_reg_stationary_dist.eps')
