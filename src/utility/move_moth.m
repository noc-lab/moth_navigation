function next_states = move_moth(state, q_state, P, Gridinfo)
...
next_states = zeros(number_next, 7);
...
for i=1:number_next
    ...
    state_next(1) = Gridinfo(1,2) + (q_state_next(1) - 1/2)*Gridinfo(1,4);
    state_next(2) = Gridinfo(2,2) + (q_state_next(2) - 1/2)*Gridinfo(2,4);
    if q_state_next(3) ==0
        state_next(3) = 0;
    else
        state_next(3) = Gridinfo(3,2) + (q_state_next(3) - 1/2)*Gridinfo(3,4);
    end
    
    next_states(i, :) = [q_state_next(1), q_state_next(2), q_state_next(3), ...
      state_next(1), state_next(2), state_next(3), p];
end
